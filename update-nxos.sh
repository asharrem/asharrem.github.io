...
function install_deb {
  ...
  if ! sudo gdebi -n -q -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 "$file_name"; then
    # Install failed
    TERM=ansi whiptail --title "$TITLE" --infobox "\n Installing $file_name failed!" 22 68
    sleep 3
    return 1
  fi
  TERM=ansi whiptail --title "$TITLE" --infobox "\n ...Installed $file_name" 22 68
  sleep 0.5
  return 0
}
...
do
  ...
  if ! sudo -S <<< "$SU_PASS" apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 update; then
...
sudo apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 install curl
...
CHOICES=$(whiptail --title "$TITLE" --separate-output --checklist "Choose options" 22 68 15 \
...
for CHOICE in $CHOICES; do
  ...
  "02")
    # Update Hostname
    TERM=ansi whiptail --title "$TITLE" --infobox "\n Updating Hostname to MAC address syntax..." 22 68
    sleep 0.5
  ...
  "04")
    ...
    # TERM=ansi whiptail --title "$TITLE" --infobox "\n Adding $USER to Chrome Remote Desktop Group..." 22 68
    # sleep 0.5
    ...
    if [ ! -f "$file_name" ]; then
      TERM=ansi whiptail --title "$TITLE" --infobox "\n Setting Chrome Browser Policy..." 22 68
      sleep 0.5
...
EOF
  ...
  "06")
    ...
    TERM=ansi whiptail --title "$TITLE" --infobox "\n Applying Nx Storage permissions Fix..." 22 68
    sleep 0.5
    ...
    if ! curl "http://admin:admin@127.0.0.1:7001/api/systemSettings?forceAnalyticsDbStoragePermissions=true"; then
      TERM=ansi whiptail --title "$TITLE" --infobox "\n Failed to apply Nx Storage permissions Fix!" 22 68
      sleep 3
    fi
  ;;
  "07")
    ...
    sudo apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 install gpg
    # advanced file support by 45drives
    curl -sSL https://repo.45drives.com/setup | sudo bash
    sudo apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 install \
    crudini \
  ...
  "08")
    ...
    sudo update-grub
    TERM=ansi whiptail --title "$TITLE" --infobox "\n Boot Splash turned OFF" 22 68
    sleep 0.5
  ;;
  "09")
    TERM=ansi whiptail --title "$TITLE" --infobox "\n Applying current freeze fixes..." 22 68
    sleep 0.5
...
sleep 0.5
sudo apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 upgrade
TERM=ansi whiptail --title "$TITLE" --infobox "\n Cleaning System..." 22 68
sleep 0.5
sudo apt -y -qq -o=Dpkg::Use-Pty=0 -o=Dpkg::Progress-Fancy=1 autoremove
